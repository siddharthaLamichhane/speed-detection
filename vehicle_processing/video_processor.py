    ...
    def __init__(self, camera_matrix: Optional[np.ndarray] = None, 
        ...
        try:
            ...
            if model_path:
                ...
                raise ValueError("YOLO_MODEL_PATH is not set in .env")

            self.model = YOLO(model_path)  # ✅ Load the model here!
    ...
    def detect_and_track_vehicles(self, video_path: str) -> List[dict]:
        ...
        while True:
            ...
            for x, y, w, h in vehicle_bboxes:
                ...
                logger.info(f"Vehicle {license_plate} position: {current_position} at {timestamp}")
